...
- Script: `Great Vibes` — couple names (`.hero-names`, `.footer-names`). (The cover line `.cover-script` is now Cormorant italic, not Great Vibes — session 14 redesign.)

## Key Animation Details

- **Cover (`.cover`) — static ivory invitation (redesigned 2026-07-11, session 14 — replaced the animated envelope):** The old wax-sealed **envelope** (flap 3D-flip + seal-lift) was **removed** — the seal was sometimes mis-positioned / read as buggy, and the user supplied a new reference (a flat ivory card). `#envelope-screen` is now an **ivory gradient** (`linear-gradient(168deg,#F5EFE6,#EFE7DB,#E8DECF)`) with two pseudo-layers: `::before` = the `images/story-vine.svg` tiled as a faint tone-on-tone **watermark** (`background-repeat:repeat`, `background-size:190px 332px`, `opacity:.2`, `mix-blend-mode:multiply`), `::after` = a soft top light sheen. `.cover` is a centred flex **column** (`gap:clamp(2.25rem,7vh,4.5rem)`) holding two children: **`.cover-inner`** (a flex **row** — the `.cover-seal` image left, the `.cover-script` line right) and the **`.cover-enter`** button below. `.cover-seal` = `images/siegel.png`, `width:clamp(108px,30vw,200px)`, static (no animation) with a drop-shadow. `.cover-script` = **Cormorant Garamond italic 600**, `clamp(1.55rem,6.5vw,3.4rem)`, deep wine `#5A1712`, text-align left, with an explicit line break layout „Diese / Einladung / ist exklusiv / **`.accent`für dich**" — the `.accent` span is champagne **gold** `#C9A86A`, `display:block`. `.cover-inner` is `max-width:min(92vw,680px)` so on **phones** it reads like the reference (seal left, big text right) and on **desktop** the whole group is capped + centred (balanced). `.cover-enter` = a **gold double-ring circle** button (`clamp(94px,24vw,122px)`, `border:1.5px gold` + inner ring via `::before`, `::after` = a faint glow ring that gently pulses via `enterIdle` to invite the tap), Cormorant italic „Enter", hover = gold-tint fill + lift, `:focus-visible` outline. It's a **native `<button>`** (keyboard Enter/Space work for free and bubble a click to the screen handler). **On tap anywhere / Enter** (`openEnvelope()`) → `.cover.opening`: `.cover-inner` fades + lifts (`opacity 0`, `scale(1.04) translateY(-8px)`, ~0.9s), `.cover-enter` runs `enterPulse` (scale up + fade, 0.75s); petals at 250ms; then the **gated reveal** (unchanged logic — see *Video Integration*): revealed only once the choreography (`CHOREO_MS` **900ms**, was 2300) has run **and** the hero video is playing, capped by `MAX_WAIT_MS` (**10000ms**). Common case: video ready by ~900ms → `#envelope-screen.closing` runs `screenFadeOut` (**0.7s**) → content reveals + scroll unlocks + music. **No more** `.envelope`/`.env-interior`/`.env-front`/`.env-flap`/`.cover-hint`/`sealBreath`/`tapFloat`.
...
1. **Cover intro** — Static **ivory invitation** (replaced the animated envelope, session 14): ivory gradient + faint vine watermark, the V&S wax seal (`siegel.png`) left, the Cormorant-italic line „Diese Einladung ist exklusiv **für dich**" (gold accent) right, and a gold double-ring **„Enter"** button below. On tap anywhere / Enter: content fades + lifts, Enter pulses out → music starts + petals fall + hero video begins playing (video-gated reveal). See the Cover bullet under *Key Animation Details*
...
> **`dietary` is now a Ja/Nein toggle, not free text (session 15).** The „Unverträglichkeiten / Allergien" **text input** was replaced by a second **button toggle** „Isst du vegetarisch?" (`#veg-group`, same `.attendance-group`/`.attendance-btn` markup + `.att-yes`/`.att-no` chip icons as „Nimmst du teil?"). It sends `dietary: "Vegetarisch" | "Nicht vegetarisch" | ""` (empty = not chosen; optional, not required). The generic toggle handler in `main.js` iterates **all** `.attendance-group`s and stores the picked value on each group's own `dataset.value` (so the two groups toggle independently); submit reads `#attendance-group`.dataset.value (required) and `#veg-group`.dataset.value (dietary). The old `#dietary`/`[name="dietary"]` input is gone. Sheet column header is unchanged.
...
**Reminder:** local CSS/JS in `index.html` are cache-busted with a `?v=YYYYMMDD` query — **bump it whenever CSS/JS changes** so phones don't serve stale files (mobile Chrome bit us before). Currently `main.js` is at `?v=20270719` (session 15 RSVP toggle); `style.css`/`animations.css`/`countdown.js` stay at `?v=20270718`; the hero **video** is `?v=20270708` after the CRF-32 recompress; favicon/OG image/audio stay at `?v=20270715`, gallery `4.jpg` + `wedding.ics` at `?v=20270716` — bump each only when that file changes — don't bump the video unless the .mp4 file actually changes, or phones needlessly re-download it.

**Update 2026-07-19 (session 15) — Final couple texts + RSVP dietary field → Ja/Nein toggle.** Two asks: (1) the couple pasted their **final wording** — 4 text tweaks in `index.html` (dresscode „Semi-formal" → „Traditionelle", ceremony lead „per Post"/„physische" dropped, **RSVP deadline „Ende 2026" → „Ende Januar 2027"**); commit `81d4784`, no cache-bust (HTML-only). The **Tagesablauf** is still open (couple will supply later). (2) Replaced the RSVP **„Unverträglichkeiten / Allergien" free-text input** with a **Ja/Nein button toggle** „Isst du vegetarisch?" mirroring the „Nimmst du teil?" toggle — see the `dietary` note in *RSVP → Google Sheets Integration*. Refactored the toggle handler to be **per-`.attendance-group`** (each group tracks its pick on its own `dataset.value`) so the two groups are independent; removed the old free-text `#dietary` input + the `attendanceBtns`/module-scope `selectedAttendance`. Optional field. Verified the toggle logic headless (both groups toggle independently, correct payload value). Cache-bust `main.js` `?v=20270718→20270719` (CSS/countdown unchanged). Commit `19d3641`.

**Update 2026-07-11 (session 14) — Cover: animated envelope → static ivory invitation.** User: the wax **seal was sometimes buggy / not on the envelope's opening spot**, and supplied a new reference (`~/Downloads/IMG_5008.JPG`) — a flat ivory card (seal left, „Diese Einladung ist exklusiv für dich" right with „für dich" in gold, a gold „Enter" ring below, tone-on-tone vine bg). Asked via `AskUserQuestion` (4 Qs) → chose: **Enter = soft fade-out + micro-zoom** (video-gated kept), **desktop = centred/balanced** (phone = reference asymmetric), **bg = reuse `story-vine.svg`** tiled, **seal = existing `siegel.png`, fully static**. Rebuilt the cover accordingly (see the Cover bullet under *Key Animation Details* for the full structure). Removed all envelope pieces (`.envelope`/`.env-interior`/`.env-front`/`.env-flap`+`::before`/`.cover-hint`/`sealBreath`/`tapFloat`); `.cover-seal` is no longer absolutely-positioned on a flap tip (that was the bug source) but a plain flex child. `openEnvelope()` kept intact incl. the gated reveal; only `CHOREO_MS 2300→900` and petals `1100→250ms`. Cover line switched Great Vibes → **Cormorant italic**. Used the **ui-ux-pro-max** skill. **Verified** headless (real `css/*`): phone (the forced-500px headless layout — see the known 500px quirk, so the seal-left/text-right fit is genuine, earlier „clipping" was just a 360/390 screenshot crop of the 500px layout) + desktop 1280 + the real `index.html` all render clean, text fully inside the viewport, seal + gold Enter ring correct. Cache-bust CSS/JS `?v=20270717→20270718` (video/favicon/OG/audio/gallery unchanged). **Open (user on-device):** exact seal size / font size / vine opacity — easy to tune.
