...
- **Section transitions:** No diagonal clip-path dividers. Content sections **strictly alternate** dark/light — `#story` dark, `#location` light, `#ceremony` dark, `#gallery` light, `#schedule` dark, `#rsvp` light, footer dark (perfect D/L/D/L rhythm so each section boundary is self-evident). The three **dark** sections (`#story`, `#ceremony`, `#schedule`) are **rounded panels** that float over the flat light sections: shared rule (in `style.css` just after the alternating-background block) gives them a **very large** `border-radius: clamp(6rem,48vw,17rem)`, `z-index:2` and **negative `margin-block`** (= −radius) so each dark panel overlaps up/down into its light neighbours and paints on top; its rounded corners cut through to reveal the lighter colour. Extra `padding-block` re-adds the overlap so inner content keeps normal spacing. `#location,#gallery,#rsvp` are `z-index:1` (paint below). Dark panels are never adjacent (alternation separates them), so no two panels overlap. `#story`'s top overlaps the **hero** (content-sheet-over-hero effect). The radius is `vw`-based so **phones** hit the 50%-width cap → top & bottom form **near-half-circle domes** (the user wanted „fast ein Halbkreis"); capped at `17rem` so full-width **desktop** panels stay strongly-rounded rather than 600px-tall domes. **Margin must never be less than −radius** or a dark sliver shows in the corner where no neighbour is behind. Works because the real light sections (Location map / Galerie marquee / RSVP form) are all tall enough that `2×radius` of overlap never collides.
