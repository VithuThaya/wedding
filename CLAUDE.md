...
4. **Location & Anfahrt** (`#location`) — Reception venue (Saalbau Kirchberg) shown as an **embedded interactive Google Map** (`<iframe class="location-map">`, `…&output=embed`, no API key) in the rounded card, badge „Reception · Afterparty", address, meta (SVG icons: date/time/dress code), `Route anzeigen` Maps link
