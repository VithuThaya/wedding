...
- **NEW known issue — gallery images 5 & 6 are rotated 90°:** `images/gallery/5.jpg` & `6.jpg` have their pixels stored sideways with **no EXIF orientation** flag (`sips -g orientation` = `<nil>`) — the earlier `sips -Z 1600` compression evidently baked/stripped rotation. Browsers can't auto-correct (no EXIF), so they likely render sideways / wrong-cropped in the marquee. Fix next session: physically rotate them upright (`sips -r 90 …` or `-r -90`, verify direction) and re-commit, or re-export from the originals in git history. (Used `gallery/3.jpg` for the OG image precisely because it's unrotated.)
