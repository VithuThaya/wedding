...
- **Section transitions:** No diagonal clip-path dividers. Content sections **strictly alternate** dark/light — `#story` dark, `#location` light, `#ceremony` dark, `#gallery` light, `#schedule` dark, `#rsvp` light, footer dark (perfect D/L/D/L rhythm so each section boundary is self-evident). The three **dark** sections (`#story`, `#ceremony`, `#schedule`) are **rounded panels** that float over the flat light sections. The radius is the token **`--panel-r: clamp(5rem,40vw,14rem)`** (`:root`). Shared rule (in `style.css` just after the alternating-background block): dark panels get `border-radius: var(--panel-r)`, `z-index:2`, **negative `margin-block`** (= −`--panel-r`) so each dark panel overlaps up/down into its light neighbours and paints on top — its rounded corners cut through to reveal the lighter colour — plus `padding-block: calc(base + var(--panel-r))` so the panel's own content clears its dome. **The light sections `#location,#gallery,#rsvp` must get the SAME `padding-block: calc(base + var(--panel-r))`** (they're `z-index:1`): the dark domes overlap `--panel-r` deep into them from BOTH sides, so without that padding the domes bulge over the light section's own content (title/map/photos) — this was a real bug (fixed same session, see session-10 note). The padding pushes the light content into a clear flat band in the middle. Radius is `vw`-based so **phones** approach the 50%-width cap → near-half-circle domes („fast ein Halbkreis"); capped at `14rem` on desktop. **Margin must stay = −`--panel-r`** or a dark sliver shows where no neighbour is behind the deep corner. `#story`'s top overlaps the **hero** (content-sheet effect). Trade-off: big domes need `~2×--panel-r` of empty vertical space at every boundary, so the page is long — reduce `--panel-r` to tighten.
