...
- The `<source>` in `index.html` carries a `?v=YYYYMMDD` cache-bust so phones don't serve a stale clip when the file is swapped (same name). Currently `?v=20270118`.
...
The gallery `<img>`s previously had inline `onerror` placeholder fallbacks — **removed 2026-07-02** now that real photos (1–6) are in. Add/remove `<figure class="gallery-item">` blocks to change count. The lightbox auto-discovers all `.gallery-item` images at load. (The placeholder images `images/placeholder/couple.jpg` & `venue.jpg` were deleted — the hero uses a themed wine gradient fallback and the location uses the embedded Google Map, so nothing references them anymore.)
...
## Expand-Map = the interactive Location card (`#location`)

The Location card **is** the animated map now — a **vanilla CSS/JS port** of a React/framer-motion component the user found (`expand-map.tsx`). It **replaced the Google-Maps `<iframe>` embed** (user's 2nd decision): the `.location-card` got class `.location-card--interactive`, the old `.location-img-wrap`/`.location-map` iframe is gone, and the animated `#expand-map` now sits in a `.location-map-stage` at the top of the card. The **real** Google Maps is still reachable via the `Route anzeigen` button (`btn-outline`) in the details body — so navigation isn't lost.

- **Combined open:** the collapsed card shows only the small map tile (240×140) + hint „Für Karte & Details tippen"; the reception details (`.location-body#location-details`) are collapsed (`max-height:0; opacity:0; padding-y:0`). **One tap/click (or Enter/Space)** on `#expand-map` toggles `.expanded` on **both** the map (→ 360×280 illustrated view) **and** the parent `.location-card` (→ body expands to `max-height:640px`, padding + opacity restored). Wired in `initExpandMap()` via `map.closest('.location-card')`.
- **Illustrated scene:** champagne-hairline roads (SVG `<line pathLength="1">` drawn via `stroke-dashoffset` `@keyframes roadDraw`), wine-tinted `.building` blocks (`buildingPop`), a **rose `.expand-map-pin`** (`pinDrop`), venue name + coordinates (`47.0918° N, 7.5290° O`), pulsing "Live" dot. Grid pattern shows only while collapsed.
- **3D tilt:** on `mousemove` (desktop only — gated by `(hover:hover) and (pointer:fine)` + not reduced-motion), `initExpandMap()` sets `rotateX/rotateY` (±8°) on `.expand-map-card`; reset on `mouseleave`/collapse. **No JS library** — framer-motion springs replaced by CSS transitions/keyframes. (Note: inside `initExpandMap` the inner `.expand-map-card` is `card`; the parent card is `locationCard` — don't shadow.)
...
- **Hint** is now visible by default (`opacity:.7`), not hover-only, since touch has no hover and it's the key affordance.
- Styles live at the **end of `css/style.css`** (`/* Decorative animated expand-map */` + `/* interactive location card */`); markup is the `.location-card--interactive` block in `#location`.
...
**Reminder:** local CSS/JS in `index.html` are cache-busted with a `?v=YYYYMMDD` query — **bump it whenever CSS/JS changes** so phones don't serve stale files (mobile Chrome bit us before). Currently at `?v=20270118`.
