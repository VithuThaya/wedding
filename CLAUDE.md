...
**Reminder:** local CSS/JS in `index.html` are cache-busted with a `?v=YYYYMMDD` query — **bump it whenever CSS/JS changes** so phones don't serve stale files (mobile Chrome bit us before). Currently at `?v=20270704`.

**Design overhaul in progress (2026-07-03, session 5) — phased, reference-image driven, DESIGN ONLY (text unchanged), gold = "Mittelweg" (glossy gold at key spots, sparing glow).** New token `--gold-metallic` (145° gold gradient for glossy fills / background-clip script names). User reviews each Etappe on mobile before the next.
- **Etappe 1 DONE** (commit `8d1c9f8`): Navbar → solid wine-gradient bar (`.site-nav.nav-scrolled`), gold uppercase links + gold underline, `.nav-cta` = filled **gold-metallic pill** with dark-wine text (now also visible on mobile). Footer → wine-gradient bg (was near-black `#1F0905`), names in **metallic gold** via `background-clip:text` (solid `--gold-soft` fallback).
- **Next Etappen (planned, awaiting per-step go):** 2 Cover (swap CSS seal → `images/siegel.png` provided by user, seal lifts on gold thread + fade on open), 3 RSVP (gold floral corner SVGs, gold-outlined fields, glossy gold submit), 4 Tagesablauf (gold-ring time circles + glass cards + line-icons, keep 5 items), 5 Story (gold vine SVG connecting empty gold photo-circles — user supplies photos, keep 4 entries).
