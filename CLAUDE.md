...
**Reminder:** local CSS/JS in `index.html` are cache-busted with a `?v=YYYYMMDD` query — **bump it whenever CSS/JS changes** so phones don't serve stale files (mobile Chrome bit us before). Currently at `?v=20270706`.
...
**Open fine-tuning candidates (user will polish):** exact seal size/position on cover; envelope-flap suggestion not added; story "vine" is a straight gold line (not the winding render); schedule icons are approximate line-SVGs; story photos still to be supplied.
